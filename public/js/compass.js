...
function size(b){
  return 42 + resonance(b) * 48
}

function opacity(b){
  return .32 + resonance(b) * .68
}
...
function toggleLink(a,b){
  ...
  const found = links.findIndex(l => l.key === key)
...
function drawManualLinks(){
  ...
  for(const link of links){
    ...
    ctx.strokeStyle = `rgba(31,111,100,${0.35 + res*.45})`
    ctx.lineWidth = (a.noteText || b.noteText ? 5 : 2.5) + res*4
    ctx.lineCap = "round"
...
function renderBubbles(){
  ...
  active.forEach(b=>{
    ...
    el.innerHTML = `
  <div class="bubble-emoji">${getEmoji(b.label, b.family)}</div>
...
function deleteBubble(b){
  ...
  if(b.noteText){
    s.personalNotes = (s.personalNotes || [])
      .filter(n => n.label !== b.label)
  }
