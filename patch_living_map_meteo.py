...
new = r'''function renderLivingResonanceMap(nodes, links, events = []){
  ...
  const points = nodes.slice(0, 18).map((n, i) => {
    ...
    const finalX = typeof n.x === "number" ? clampX(n.x) : w / 2
    const finalY = typeof n.y === "number" ? clampY(n.y) : h / 2

    const startRadius = 105
    const startX = w / 2 + Math.cos(angle) * startRadius
    const startY = h / 2 + Math.sin(angle) * 82

    const c = centrality[n.id] || 0
    const r = 15 + Math.min(c * 5, 18)

    const values = `
      ${startX} ${startY};
      ${finalX} ${finalY};
      ${finalX + Math.sin(i + 1) * 8} ${finalY + Math.cos(i + 2) * 8};
      ${finalX} ${finalY};
      ${startX} ${startY}
    `
    ...
    <svg viewBox="0 0 ${w} ${h}" class="living-map timelapse-map" role="img" aria-label="Timelapse du paysage vivant de résonances">
      <defs>
        <radialGradient id="livingGlow" cx="50%" cy="50%" r="70%">
          ...
          <stop offset="50%" stop-color="rgba(150,185,255,.24)" />
      ...
      ${points.map((p, i) => `
        <path
          class="living-attractor"
          d="M ${p.startX} ${p.startY} Q ${w/2} ${h/2} ${p.x} ${p.y}"
          style="animation-delay:${i * .18}s"
        />
      `).join("")}
      ...
      ${links.map((l, i) => {
        ...
        return `
          <line x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}" class="living-link">
            <animate attributeName="opacity" values=".08;.72;.08" dur="${4 + i % 3}s" repeatCount="indefinite"/>
      ...
      ${points.map((p, i) => `
        <g class="living-node" transform="translate(${p.x} ${p.y})">
          ...
          <circle class="living-halo" r="${p.r + 8}">
            <animate attributeName="r" values="${p.r + 4};${p.r + 18};${p.r + 4}" dur="${4 + i % 4}s" repeatCount="indefinite"/>
            <animate attributeName="opacity" values=".10;.38;.10" dur="${4 + i % 4}s" repeatCount="indefinite"/>
          </circle>
